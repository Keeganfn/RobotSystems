...
class Picarx(object):
    ...
    def __init__(self, 
        ...
        self.sensors = picarx_sensors.PicarxSensors(grayscale_pins, ultrasonic_pins)
    ...
    def forward(self,speed):
        ...
        if current_angle != 0:
            ...
            # power_scale = (100 - abs_current_angle) / 100.0
            # print("power_scale:",power_scale)
            #outer_wheel = (speed / (2*math.pi*.066)) * ((180-self.dir_current_angle) + (.1075/2)) / ((.1)) 
            outer_wheel = 1.5 * speed
            if (current_angle / abs_current_angle) > 0:
                self.set_motor_speed(1, 1*outer_wheel)
                self.set_motor_speed(2, -speed) 
                # print("current_speed: %s %s"%(1*speed * power_scale, -speed))
            else:
                self.set_motor_speed(1, speed)
                self.set_motor_speed(2, -1*outer_wheel)
